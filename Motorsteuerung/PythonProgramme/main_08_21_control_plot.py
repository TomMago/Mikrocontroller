...
class Controlpanel(wx.Panel):
    ...
    def __init__(self, parent):
        ...
        global MotorSet
        MotorSet=[0,0,0,0] #Motor[0] links vorne, Motor [1] rechts vorne, Motor[3] links hinten, Motor[2] rechts hinten
        global Direction
        ...
        global left
        left=wx.ToggleButton(self,-1,pos=(12,58),size=(48,48),style = wx.NO_BORDER)
        left.SetBitmap(arrowleftgrey)
        left.SetBitmapPressed(arrowleftblack)
        global right
        right=wx.ToggleButton(self,-1,pos=(108,58),size=(48,48),style = wx.NO_BORDER)
        right.SetBitmap(arrowrightgrey)
        right.SetBitmapPressed(arrowrightblack)
        global up
        up=wx.ToggleButton(self,-1,pos=(60,10),size=(48,48),style = wx.NO_BORDER)
        up.SetBitmap(arrowupgrey)
        up.SetBitmapPressed(arrowupblack)
        global down
        down=wx.ToggleButton(self,-1,pos=(60,58),size=(48,48),style = wx.NO_BORDER)
        down.SetBitmap(arrowdowngrey)
        down.SetBitmapPressed(arrowdownblack)
        global park
        park=wx.ToggleButton(self,-1,pos=(108,10),size=(48,48),style = wx.NO_BORDER)
...
class MatplotPanel1(wx.Panel):
    ...
    def __init__(self,parent):
        ...
        self.ax.set_xticklabels(('Motor 0','Motor 1'))
        plt.ylabel('Umdrehungen pro Sekunde')
        self.line, = self.ax.plot(motor,marker='o',markersize=20,linewidth=0,color='r',markeredgecolor='k',markeredgewidth='5')
        ...
        self.ani = animation.FuncAnimation(self.fig, self.OnPaint, self.receive, init_func=self.init_ani, interval=500,blit=True)
        #plt.show()
        #plt.ion()
...
class MatplotPanel2(wx.Panel):
    ...
    def __init__(self,parent):
        ...
        global theta
        theta= [0*np.pi,1*np.pi/2,2*np.pi/2,3*np.pi/2]
        self.fig = plt.figure()
        #Polarplot
        self.ax=self.fig.add_subplot(111,projection='polar')
        self.ax.set_xticks(theta)
        self.line, = self.ax.plot(theta,us,marker='o',markersize=10,linewidth=0)
        self.ani = animation.FuncAnimation(self.fig, self.OnPaint, self.receive, init_func=self.init_ani, interval=500,blit=True)
        #*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*
        #moeglichst block=True, damit Einbindung in Fenster. Falls das nicht klappt: Block=False
        plt.show(block=False)
...
class MatplotPanel3(wx.Panel):
    ...
    def compute(self):
        ...
        global Direction
        global lastCommand
        #Aenderung des Motorwerts pro Iteration
        MotorChange=4
        ...
        while True:
            ...
            if lastCommand[1]=='a':
                if (Direction==1):
                    #Lenkung
                    if (MotorSet[0]>=MotorChange)&(MotorSet[0]<=MotorSet[1]):
                        MotorSet[0]=MotorSet[0]-MotorChange
                ...
                if (Direction==1):
                    if (MotorSet[1]>=MotorChange)&(MotorSet[1]<=MotorSet[0]):
                        MotorSet[1]=MotorSet[1]-MotorChange
                ...
                if Direction==-1:
                    if (MotorSet[1]>MotorSet[0]):
                        ...
                        MotorSet[0]=MotorSet[0]-MotorChange
                #nach Sprung von d->a oder andersherum, wenn Nullpunkt durchlaufen, Lenkung fortsetzen
                if tempChar!='0':
                    if MotorSet[1]==MotorSet[0]:
    ...
    def __init__(self,parent):
        ...
        plt.ylabel('Richtung')
        self.ax.grid(True)
        self.line, = self.ax.plot(MotorSetx,MotorSety,marker='s',markersize=16,linewidth=0,color='firebrick',markeredgecolor='k',markeredgewidth='2')
        #*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*
        #Interval aendern, um schneller oder langsamer Motorwert zu uebertragen
        self.ani = animation.FuncAnimation(self.fig, self.OnPaint, self.compute, init_func=self.init_ani, interval=500,blit=True)
        #*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*
        #moeglichst block=True, damit Einbindung in Fenster. Falls das nicht klappt: Block=False
        plt.show(block=False)
